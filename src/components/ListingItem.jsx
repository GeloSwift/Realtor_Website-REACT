...
export default function ListingItem({ listing, id, onEdit, onDelete }) {
  return (
    <li
      className="
        relative bg-white flex flex-col justify-between items-center 
        shadow-md hover:shadow-xl rounded-md overflow-hidden 
        transition-shadow duration-150 m-2.5
      "
    >
      <Link to={`/category/${listing.type}/${id}`} className="contents">
        <img
          ...
          loading="lazy"
          className="
            h-44 w-full object-cover hover:scale-105
        ...
        <Moment
          fromNow
          className="
            absolute top-2 left-2 bg-[#3377cc] text-white 
            uppercase text-xs font-semibold rounded-md 
            px-2 py-1 shadow-lg
          "
        >
      ...
      {onEdit && (
        <MdEdit
          className="absolute bottom-2 right-7.5 h-4 cursor-pointer"
